...
<div class='oa-list oa-discussion-topic well clearfix <?php print $current_class; ?> <?php print $nid_class; ?>'>
  ...
  <div class='oa-list-header'>
    ...
    <?php
      if(!empty($row->field_og_group_ref[0]['rendered']['#markup'])){
        $path = explode('/', drupal_get_path_alias($_GET['q']));
        if(strtolower($path[sizeof($path)-1]) == 'discussions'){
          unset($path[sizeof($path)-1]);
        }
        print ' in ' . l($row->field_og_group_ref[0]['rendered']['#markup'], implode('/',$path).'/'.strtolower($row->field_og_group_ref[0]['rendered']['#markup']));
      }
    ?>
    <?php
  ...
  <div class='oa-list-header oa-description' style="text-align:right;">
